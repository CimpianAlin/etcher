...
describe('Shared: s3Packages', function() {
  ...
  describe('.getLatestVersion()', function() {
    ...
    describe('given a greater production version in a snapshot bucket', function() {
      ...
      it('should ignore production versions', function(done) {
        s3Packages.getLatestVersion(s3Packages.BUCKET_URL.SNAPSHOT).then((latestVersion) => {
  ...
  describe('.getLatestVersionFromReleaseType()', function() {

    beforeEach(function() {
      this.getLatestVersionStub = m.sinon.stub(s3Packages, 'getLatestVersion');

      this.getLatestVersionStub
        .withArgs(s3Packages.BUCKET_URL.PRODUCTION)
        .returns(Bluebird.resolve('1.0.0'));

      this.getLatestVersionStub
        .withArgs(s3Packages.BUCKET_URL.SNAPSHOT)
        .returns(Bluebird.resolve('2.0.0'));
    });
    ...
    it('should pass the options to getLatestVersion', function(done) {
      const options = {
        range: '<2.0.0'
      };

      s3Packages.getLatestVersionFromReleaseType(release.RELEASE_TYPE.PRODUCTION, options).then(() => {
        m.chai.expect(this.getLatestVersionStub).to.have.been.calledWith(s3Packages.BUCKET_URL.PRODUCTION, options);
        done();
      }).catch(done);
    });
    ...
    describe('given a production release type', function() {
      ...
      it('should call getLatestVersion with the production bucket URL', function(done) {
        s3Packages.getLatestVersionFromReleaseType(release.RELEASE_TYPE.PRODUCTION).then((latestVersion) => {
          m.chai.expect(latestVersion).to.equal('1.0.0');
          done();
        }).catch(done);
      });
    ...
    describe('given a snapshot release type', function() {
      ...
      it('should call getLatestVersion with the snapshot bucket URL', function(done) {
        s3Packages.getLatestVersionFromReleaseType(release.RELEASE_TYPE.SNAPSHOT).then((latestVersion) => {
          m.chai.expect(latestVersion).to.equal('2.0.0');
          done();
        }).catch(done);
      });
    ...
    describe('given an unknown release type', function() {

      it('should be rejected with an error', function(done) {
        const unknownReleaseType = release.RELEASE_TYPE.UNKNOWN;
        s3Packages.getLatestVersionFromReleaseType(unknownReleaseType).catch((error) => {
          m.chai.expect(error).to.be.an.instanceof(Error);
          m.chai.expect(error.message).to.equal(`No bucket URL found for release type: ${unknownReleaseType}`);
          done();
        });
      });
    ...
    describe('given a not recognised release type', function() {

      it('should be rejected with an error', function(done) {
        s3Packages.getLatestVersionFromReleaseType('foobar').catch((error) => {
          m.chai.expect(error).to.be.an.instanceof(Error);
